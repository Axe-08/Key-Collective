<script lang="ts">
  ...
  let { toasts, onDismiss }: { toasts: ToastMessage[]; onDismiss: (id: string) => void } = $props();
</script>

<div class="fixed bottom-4 right-4 z-50 flex flex-col gap-2 max-w-sm w-full pointer-events-none">
  {#each toasts as toast (toast.id)}
    <div
      class="pointer-events-auto flex items-start gap-3 p-3.5 rounded-xl border shadow-xl backdrop-blur-md transition-all duration-300 transform translate-y-0 text-sm {toast.type === 'success'
    ...
    >
      <div class="mt-0.5 shrink-0">
        ...
        {:else}
          <svg class="w-4 h-4 text-sky-400" viewBox="0 0 20 20" fill="currentColor">
            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z" clip-rule="evenodd" />
          </svg>
        {/if}
      </div>
      <div class="flex-1 font-medium leading-snug break-words">
        {toast.message}
      </div>
      <button
        type="button"
        onclick={() => onDismiss(toast.id)}
        class="text-slate-400 hover:text-white transition-colors p-0.5 rounded"
